...
var DOPBSPAddons = new function(){
    ...
    this.display = function(){
        ...
        $.post(ajaxurl, {action: 'dopbsp_addons_display'}, function(data){
            ...
            if (data === 'error'){
                DOPBSP.toggleMessages('error', DOPBSP.text('ADDONS_LOAD_ERROR'));
            }
            else{
                content = data.split(';;;;;;;');
                ...
                DOPBSP.toggleMessages('success', DOPBSP.text('ADDONS_LOAD_SUCCESS'));
            }
        }).fail(function(data){
    ...
    this.category = function(key){
        ...
        DOPBSP.toggleInputs('addons-category-'+key);
    };
